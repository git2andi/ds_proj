...
1. **`builders.py`** — two sequential LLM calls: first generates option cards (with `shared_context` and `short_name`), then per-persona hidden belief state. The controller samples only the five OCEAN traits. Response length and compromise willingness are derived deterministically from them; stubborn profiles come from separate five-trait ranges when `hard_blocker_probability` activates. Preference setup is still pending simplification under KF23.
