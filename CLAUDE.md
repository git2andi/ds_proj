...
5. **`scoring.py`** — shared lean/support calculations plus transcript-visible preference and candidate-status classification used by outcomes and moderator prompts.
...
VOTE and ACCEPT guidance asks for a named option plus an explicit first-person choice or acceptance, not a vague preference, formal ballot, or process announcement. The validator normalizes typographic apostrophes and accepts equivalent choice/acceptance wording across both routed acts, while uncertain or conditional language remains neutral. COMPARE, SUPPORT, REACT, and OBJECT operate on the local point and explicitly avoid biography restatement or a fresh option pitch.
...
- **Commitment gating**: `accept`/`vote`/`reject` only count as binding on routed decision turns (narrowing/confirmation), not during free discussion. A visible commitment must name one option and use explicit first-person choice or acceptance wording; vague praise and noun-only preferences do not count. Uncertain or conditional votes and accepts are clamped to neutral.
- **Repair integrity**: when a rewrite introduces a state-blocking issue into an otherwise state-valid turn, keep the original provider-written line and log its original issues rather than losing a real commitment.
...
- **Outcome taxonomy**: finalization scans every option; `successful` = every participant visibly voted/accepted the same option, `majority` = one option has ≥66% visible support, and `unresolved` = no option met either threshold. Only explicit `explicit_vote` and `accepted_options` entries count — the controller candidate, hidden preferences, and routing leans cannot select the outcome.
...
- **Moderator state accuracy**: moderator summaries use transcript-visible preferences, not mutable controller leans. Pending participants are classified as visible supporters, actual alternative/objection holders, or missing explicit commitments; the last group receives a confirmation request rather than resistance language. Majority/unresolved closures name visible supporters, non-supporters, and missing commitments. `_MODERATOR_RULES` forbids invented votes and merging fixed options; `_moderator_say()` retries incomplete sentences.
