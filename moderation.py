...
Extracted from orchestrator so the state-machine and the intervention
logic live in separate places.
...
class ModerationEngine:
    ...
    def __init__(
        ...
        self.sims = sims          # shared reference — add_sim mutations propagate
    ...
    def _detect_speculative_loop(
        ...
        option description for 4+ consecutive turns. Returns None in open mode.
        """
    ...
    def _detect_outlier(
        ...
        for sim in self.sims:
            turns = self._last_n_turns_for(sim.name, history, n=2)
            if len(turns) < 2:
                continue
    ...
    def _current_votes(self, history: list[str]) -> dict[str, str]:
        votes: dict[str, str] = {}
        for line in reversed(history):
            if ":" not in line:
                continue
