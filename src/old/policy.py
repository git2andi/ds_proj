...
def select_next_speakers(
    ...
    # Directed participant questions get one response slot.
    if state.pending_question_target:
        targeted = [s for s in sims if s.name == state.pending_question_target]
        if targeted:
            return [targeted[0]]
...
def extract_discourse(history: list[str], sim_names: set[str]) -> dict[str, Optional[str]]:
    ...
    for name in sim_names:
        if name == speaker:
            continue
        if _is_addressing(name, msg, is_question=is_question):
...
def _is_addressing(name: str, text: str, *, is_question: bool) -> bool:
    n = re.escape(name)
    if re.search(rf"\b(?:with|like|as|what)\s+{n}\b|{n}'s\b", text, re.I):
        return False
