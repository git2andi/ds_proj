...
# defaults + serialization, and thread-routing trace fields.
class PhaseAndThreadModelTests(unittest.TestCase):
    ...
    def test_thread_and_repair_state_serialize_to_json(self):
        from logger import _to_jsonable
        thread = ThreadState(
            thread_id="t1", thread_type=ThreadType.QUESTION, focus_options=["A"],
            question_scope="direct", blocking_strength=BlockingStrength.SOFT,
        )
    ...
    def test_trace_routed_thread_is_none_for_normal_turns(self):
        ...
        random.seed(12)
        runner._generate_and_append(state, MoveIntent(
            speaker_id="p1", act=ActType.SUPPORT, reason="support", option_focus=["A"],
        ))
        entry = [e for e in state.controller_trace if e["type"] == "turn"][0]
