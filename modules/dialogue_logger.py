"""
modules/dialogue_logger.py
--------------------------
Handles all output for a single dialogue run: writing the txt transcript
and buffering/flushing the CSV data file.

Extracted from orchestrator.py so that orchestrator only coordinates
dialogue flow and delegates I/O here.
"""
...
CSV_COLUMNS = [
    ...
    "pending_reply_target", "repetition_pressure",
    "role", "is_primary",
    "friendliness", "assertiveness", "talkativeness", "initiative",
    "agreeableness", "flexibility", "patience", "response_length",
    "contrarian_pressure",
    "focus_cost", "focus_comfort", "focus_time", "focus_safety", "focus_flexibility",
]

_PERSONA_FIELDS = [
    "role", "is_primary", "friendliness", "assertiveness", "talkativeness",
    "initiative", "agreeableness", "flexibility", "patience", "response_length",
    "contrarian_pressure",
]
...
class DialogueLogger:
    ...
    def buffer(self, line: str, selected_reason: str, state: "DialogueState", sims: list) -> None:
        ...
        row: dict[str, Any] = {
            ...
            "pending_reply_target": state.pending_reply_target or "",
            "repetition_pressure": round(state.repetition_pressure, 3),
            **{col: "" for col in CSV_COLUMNS if col not in {
                "dialogue_id", "turn_index", "phase", "speaker", "is_moderator",
                "text", "selected_reason", "last_addressed",
                "pending_question_target", "pending_reply_target", "repetition_pressure",
            }},
        }
        ...
        if not is_moderator:
            ...
            if sim:
                ...
                row.update({field: p.get(field, "") for field in _PERSONA_FIELDS})
                row.update({
