...
KF02 resolved: outcome taxonomy is now `successful / majority / unresolved`. `support_fraction()` counts only visible `explicit_vote` and `accepted_options`, not hidden `current_preference` or `preferred_option`. Updated `dialogue.py`, `prompts.py`, `evals/run_eval.py`, `tests/test_consensus.py`.
