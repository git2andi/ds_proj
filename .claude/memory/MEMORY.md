...
- [Naturalness history and current routing](ds-proj-naturalness-plan.md) — S1-S4 are historical; current priorities live only in known_failures
- [Prompt length feedback](feedback-prompt-length.md) — shorter prompts, not longer; llama3.3 ignores excess rules
- [Implementation process feedback](feedback-implementation-process.md) — one independently verified issue per upgrade; synchronize all active info files; no forced naturalness routing
- [Provider rule](feedback-uni-provider.md) — use only the provider explicitly authorized for the current task; current sequence authorizes GPT
