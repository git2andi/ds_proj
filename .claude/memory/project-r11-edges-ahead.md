...
**How to apply:** Fixed in three places — (1) COMPARE guidance in `src/prompts.py` rewritten to description only; (2) `re.compile(r"\bedges?\s+ahead\b", re.I)` added to `_ROBOTIC_TEMPLATES` in `src/validation.py`; (3) 'edges ahead' added to banned stock-phrases list in rule 3 of `sim_utterance`. Test case added to `tests/test_validation.py`. When writing COMPARE guidance in future, describe the move ("acknowledge theirs, then give your reason") without using any competitive-comparison phrase.
