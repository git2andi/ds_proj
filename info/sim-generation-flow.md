...
A **social greeting round** (`_social_round`) picks extraverted personas (proportional to extraversion/max), each says a brief greeting line generated by `prompts.greeting_line()`. Cosmetic only — greetings do not affect stance or coverage state.
...
3. **Moderator intervention** (rate-limited, optional) — `_moderator_intervention()` may fire a stall nudge (if discussion circled without progress) or a holdout nudge (if 1–2 people haven't accepted the candidate in CONFIRMATION). The LLM generates moderator text via `_moderator_say()`.
...
6. **State update** — `StateTracker.apply_participant()`:
   ...
   - `_update_coverage()`: records option mentions, reasons (by word-count proxy), objections, acceptances, and covered claim slots.
...
- `majority` — candidate reached `majority_fallback_fraction` of visible support.
- `unresolved` — neither threshold met.

The moderator says a closure line (LLM-generated). A **farewell social round** mirrors the greeting: extraverted personas say brief goodbyes, each shown prior goodbye lines to avoid repetition.
